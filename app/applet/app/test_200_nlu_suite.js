...
function getMatchedProducts(query, products) {
  ...
  const FOOD_ALIASES = [
    { keywords: ['nattu kozhi', 'nattukoli', 'naattu kozhi', 'country chicken'], terms: ['country chicken', 'nattu kozhi', 'நாட்டுக்கோழி'], scoreWeight: 800 },
    { keywords: ['chicken', 'சிக்கன்', 'சிகன்', 'கோழி', 'broiler', 'koli', 'chikan', 'chikkan'], terms: ['chicken', 'சிக்கன்', 'broiler'], scoreWeight: 600 },
    { keywords: ['mutton', 'மட்டன்', 'ஆடு', 'ஆட்டிறைச்சி', 'கறி', 'goat', 'lamb', 'matton'], terms: ['mutton', 'மட்டன்', 'goat', 'lamb'], scoreWeight: 600 },
    { keywords: ['fish', 'மீன்', 'சங்கரா', 'வஞ்சரம்', 'prawn', 'நண்டு', 'crab', 'meen'], terms: ['fish', 'மீன்', 'sankara', 'vanjaram', 'seafood'], scoreWeight: 600 },
    { keywords: ['egg', 'முட்டை', 'முட்ட', 'eggs', 'muttai'], terms: ['egg', 'முட்டை', 'eggs'], scoreWeight: 600 },
    { keywords: ['tomato', 'தக்காளி', 'தக்காலி', 'thakkali', 'thakali'], terms: ['tomato', 'தக்காளி'], scoreWeight: 600 },
    { keywords: ['onion', 'வெங்காயம்', 'வெங்காய', 'vengayam', 'vengaayam'], terms: ['onion', 'வெங்காயம்'], scoreWeight: 600 },
    { keywords: ['potato', 'உருளைக்கிழங்கு', 'உருளை', 'urulai', 'urulaikilangu'], terms: ['potato', 'உருளைக்கிழங்கு', 'உருளை'], scoreWeight: 600 },
    { keywords: ['garlic', 'பூண்டு', 'poondhu', 'poondu'], terms: ['garlic', 'பூண்டு'], scoreWeight: 600 },
    { keywords: ['milk', 'பால்', 'paal'], terms: ['milk', 'பால்'], scoreWeight: 600 },
    { keywords: ['oil', 'எண்ணெய்', 'ennai', 'sunflower'], terms: ['oil', 'எண்ணெய்', 'sunflower'], scoreWeight: 600 },
    { keywords: ['paneer', 'பன்னீர்'], terms: ['paneer', 'பன்னீர்'], scoreWeight: 600 }
  ];
  ...
  activeProducts.forEach(p => {
    ...
    qTokens.forEach(t => {
      const isWordMatch = new RegExp('\\b' + t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'i');
      if (isWordMatch.test(engName) || isWordMatch.test(tamName)) {
        score += 300;
      } else {
        if (t.length > 3 && engName.includes(t)) score += 100;
        if (t.length > 3 && tamName.includes(t)) score += 100;
      }
    });

    FOOD_ALIASES.forEach(aliasObj => {
      const queryMatchesAlias = aliasObj.keywords.some(kw => {
        if (kw.length <= 3) return new RegExp('\\b' + kw + '\\b', 'i').test(q);
      ...
      if (queryMatchesAlias) {
        ...
        if (productMatchesAlias) score += aliasObj.scoreWeight;
      }
    });

    // Bonus for Broiler Chicken when no "country" or "nattu" in query
    if (engName.includes('broiler') && !q.includes('nattu') && !q.includes('country') && !q.includes('நாட்டு')) {
      score += 200;
    }
...
function parseOfflineOrderProposal(userQuery, activeProducts, isTamil, honorific = "அண்ணே") {
  ...
  segments.forEach(seg => {
    ...
    const moneySuffixMatch = trimmedSeg.match(/(\d+(?:\.\d+)?)\s*(?:₹|rs\.?|inr|rupees?|rupee|rupai|rupaiye|rupia|bucks|ரூபாய்க்கு|ரூபாயிக்கு|ரூபாய்|ரூபாயி|ரூ|rupaaiku|rupaai|rskku|rsku|rs)/i);
    ...
    if (moneySuffixMatch) {
      ...
      if (!isExplicitFound) {
        if (trimmedSeg.includes("ஒன்றரை") || trimmedSeg.includes("ஒன்னரை") || trimmedSeg.includes("onnarai") || trimmedSeg.includes("1.5")) {
          value = 1.5;
          unitType = "WEIGHT_KG";
          isExplicitFound = true;
        } else if (trimmedSeg.includes("இரண்டரை") || trimmedSeg.includes("rendarai") || trimmedSeg.includes("2.5")) {
          value = 2.5;
          unitType = "WEIGHT_KG";
          isExplicitFound = true;
        } else if ((trimmedSeg.includes("அரை") || trimmedSeg.includes("arai") || trimmedSeg.includes("half")) && !trimmedSeg.includes("ஒன்றரை") && !trimmedSeg.includes("ஒன்னரை") && !trimmedSeg.includes("onnarai") && !trimmedSeg.includes("இரண்டரை") && !trimmedSeg.includes("rendarai")) {
          value = 0.5;
          unitType = "WEIGHT_KG";
          isExplicitFound = true;
        } else if (trimmedSeg.includes("முக்கால்") || trimmedSeg.includes("mukkall") || trimmedSeg.includes("mukkál")) {
          value = 0.75;
          unitType = "WEIGHT_KG";
          isExplicitFound = true;
        } else if (trimmedSeg.includes("கால்") || trimmedSeg.includes("kaal") || trimmedSeg.includes("quarter")) {
          value = 0.25;
          unitType = "WEIGHT_KG";
          isExplicitFound = true;
        } else {
    ...
    let cleanQuery = trimmedSeg
      .replace(/(?:\d+\s+)?\d+\/\d+/g, ' ')
      .replace(/\d+(?:\.\d+)?/g, ' ')
      .replace(/(kg|kgs|kilo|kilos|kilograms?|grams?|gram|gms?|g|rs\.?|rupees|rupee|₹|pcs|piece|pieces|packets?|packet|bunches?|bunch|dozens?|liters?|litres?|liter|litre|ml|அரை|கால்|முக்கால்|கிலோ|கீலோ|கிராம்|டஜன்|பீஸ்|பீஸ்கள்|பாக்கெட்|பாக்கெட்டுகள்|கட்டு|கட்டுகள்|லிட்டர்|ரூபாய்|ரூ|ரூபிஸ்|ரூபாய்க்கு|ரூபாயி|ரூபாயிக்கு|rupaaiku|rupaai|rupai|rskku|rsku|rs|worth|and|மற்றும்|அப்புறம்|அதோடு|arai|kaal|mukkall|onnarai|rendarai)/gi, ' ')
...
module.exports = { mockProducts, getMatchedProducts, parseOfflineOrderProposal };
